...
"""XGBOOST"""
def xgboost_classifier(corpus, ml, dst_path):
    ...
    # Single class into multi-target:
    if ml == 'binary':
        multi_target_xgb_model = MultiOutputClassifier(xgb, n_jobs=-1)
        # Parameter(s) to optimize:
        params = {'estimator__n_estimators' : list(map(int, np.linspace(100, 1000, 10)))}
    elif ml == 'powerset':
        multi_target_xgb_model = LabelPowerset(classifier = xgb,
            require_dense=[False, False])
        # Parameter(s) to optimize:
        params = {'classifier__n_estimators' : list(map(int, np.linspace(100, 1000, 10)))}
    ...
    # Grid search
    multi_target_forest = GridSearchCV(multi_target_xgb_model, params, cv = ps, n_jobs = -1, scoring = average_precision_wrapper)

    # Train model:
    multi_target_forest.fit(X_GridSearch, np.array(y_GridSearch, dtype=int))

    # Persisting the model:
    dump(multi_target_forest.best_estimator_, os.path.join(dst_path, 'xgboost_{}.cls'.format(ml)))
