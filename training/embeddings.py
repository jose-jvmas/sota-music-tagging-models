...
class Predict(object):
    ...
    def get_embeddings(self):
        ...
        if config.partition == 'train':
            partition = self.train_list
        elif config.partition == 'valid':
            partition = self.valid_list
        elif config.partition == 'test':
            partition = self.test_list
    ...
    def save_embedding(self, emb, gt, corpus, model_name, partition):
        base_path = 'embeddings'

        if not os.path.exists(base_path): os.makedirs(base_path)
        if not os.path.exists(os.path.join(base_path, corpus)): os.makedirs(os.path.join(base_path, corpus))
        if not os.path.exists(os.path.join(base_path, corpus, model_name)): os.makedirs(os.path.join(base_path, corpus, model_name))

        with open(os.path.join(base_path, corpus, model_name, partition + '_emb.csv'), 'a+') as fout:
            fout.write(",".join([str(u) for u in emb]) + '\n')

        with open(os.path.join(base_path, corpus, model_name, partition + '_gt.csv'), 'a+') as fout:
            fout.write(",".join([str(int(u)) for u in gt]) + '\n')
